...
app.controller('survivalController', function ($scope, $http) {
    $scope.load = function () {
        $http.get('../../data/survival.json').success(function (data) {

            $scope.data = data;
            var strata1 = data.filter(function (d) {
               return d.Strata == 1;
            });
            var strata2 = data.filter(function (d) {
               return d.Strata == 2;
            });
            engagement(data,'miles1','All Strata');
            engagement(strata1, 'strata1','Strata-1');
            engagement(strata2, 'strata2','Strata-2');

        });
        var engagement = function(data, id, title) {
            var margin = {top: 30, right: 90, bottom: 45, left: 50},
    ...
    $scope.load1 = function () {
        ...
        var churn = function (data1, id, title) {
            var margin = {top: 50, right: 80, bottom: 45, left: 50},
                width = $('#'+id).width() - margin.left - margin.right,
